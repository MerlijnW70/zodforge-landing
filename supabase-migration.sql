...
-- 3. Add index on api_key_hash for faster lookups
CREATE INDEX IF NOT EXISTS idx_customers_api_key_hash
ON customers(api_key_hash);

-- 4. Add created_at index for analytics
...
-- 6. Make api_key_hash required for new customers (only after migration)
-- Run this AFTER all existing customers have been migrated
-- ALTER TABLE customers
